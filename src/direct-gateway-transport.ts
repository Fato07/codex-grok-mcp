...
function endpoint(config: DirectGatewayConfig, command: "listAgents" | "sendPrompt"): string {
...
function httpError(
  status: number,
  requestId: string,
  command: "listAgents" | "sendPrompt",
): GrokBotGatewayError {
  const definitelyRejected = [400, 401, 403, 404, 413, 422].includes(status);
  const deliveryMayHaveOccurred = command === "sendPrompt" && !definitelyRejected;
  if (status === 401 || status === 403) {
    return error("AUTH_FAILED", "Grok Bot gateway authentication failed.", { requestId });
  }
  if (status === 429) {
