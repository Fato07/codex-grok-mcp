...
The send completion boundary `gateway_accepted_not_bot_reply` is not proof of a Bot reply, task completion, or message persistence. The gateway protocol is unofficial and unsupported by xAI; compatibility and security behavior may change without notice. A metadata-only live VM probe verified discovery, authentication, and full-roster access. No live paired-read, paired-message, or background-lifecycle validation is claimed for this beta.
